...
int main(int argc, char** argv)
{
    ...
    std::vector<VkPhysicalDevice> physical_devices(n_devices);
    result = vkEnumeratePhysicalDevices(vk_instance, &n_devices, physical_devices.data());
    uint32_t physical_device_idx = hlpSelectPhysicalDeviceIndex(physical_devices, vk_surface);
    vk_physical_device = physical_devices.at(physical_device_idx);
    VKL_LOG(vk_physical_device);
    ...
    VkSurfaceFormatKHR surf_format = hlpGetSurfaceImageFormat(vk_physical_device, vk_surface);
    swapchain_create_info.imageFormat = VK_FORMAT_R8G8B8A8_UNORM;
    swapchain_create_info.imageColorSpace = surf_format.colorSpace;
    swapchain_create_info.queueFamilyIndexCount = 1;
    uint32_t queue_family_indices[] = { selected_queue_family_index };
    ...
    vkGetSwapchainImagesKHR(vk_device, vk_swapchain, &n_swapchain_images, nullptr);
    result = vkGetSwapchainImagesKHR(vk_device, vk_swapchain, &n_swapchain_images, swap_chain_images.data());
    VKL_CHECK_VULKAN_RESULT(result);
    ...
    for (VkImage vk_image : swap_chain_images) {
        ...
        framebufferData.colorAttachmentImageDetails.clearValue.color = { 0.39f, 0.58f, 0.93f, 1.0f };
...
uint32_t selectQueueFamilyIndex(VkPhysicalDevice physical_device, VkSurfaceKHR surface)
{
    ...
    for (uint32_t i = 0; i < queue_family_count; i++) {
        ...
        if ((queue_family.queueFlags & VK_QUEUE_GRAPHICS_BIT) == VK_QUEUE_GRAPHICS_BIT) {
            ...
            if (vkGetPhysicalDeviceSurfaceSupportKHR(physical_device, i, surface, &is_presentation_supported) == VK_SUCCESS) {
                return i;
            }
        }
        return -1;
